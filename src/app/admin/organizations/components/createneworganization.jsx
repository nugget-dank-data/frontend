      ...
      <div className="bg-white flex flex-col min-w-[40%] rounded-lg float-right text-center m-24 items-center  ">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col m-4 p-3 gap-3">
          ...
          <div className="subsidaryaccount rounded-md bg-[#9d9d9d3b] flex flex-col">
            <label className="text-left w-full bg-red-200 float-left text-[1.3em] justify-start" htmlFor="Organizationowner">
              Subsidiary Accounts
            </label>
            <div className="flex flex-col">
              {subsidiaryAccounts.map((account) => (
                <div key={account.id} className="flex flex-col p-4 gap-4">
                  ...
                  <span className="rounded-lg flex items-center px-2 h-[2em] shadow-2xl border">#{account.id}</span>
                  <input
                    type="email"
                    ...
                    onChange={(e) => {
                      const updatedAccounts = subsidiaryAccounts.map((acc) =>
                        acc.id === account.id ? { ...acc, email: e.target.value } : acc
                      );
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const updatedAccounts = subsidiaryAccounts.map((acc) =>
                        acc.id === account.id ? { ...acc, role: e.target.value } : acc
                      );
                      setSubsidiaryAccounts(updatedAccounts);
                    }}
                  >
          ...
          <div className="p-2 m-auto items-start ml-0 rounded-lg float-left text-[#4d0798] bg-[#9d9d9d3b] flex " onClick={handleAddSubsidiary}>
          <BsPlus value={{ style: { className: 'bg-blue-300 w-8' }}} size={'1.3em'}/>
            add
          </div>
          ...
          <label className="text-left justify-start" htmlFor="plans">
              ...
              <select name="plans" id="plans" className=" text-[#858585cb] rounded-lg p-4 bg-white border" placeholder="select a plan">
          ...
          type="submit"
            ...
            className="text-white justify-center text-[1.2em] items-center py-4 rounded-lg text-center bg-[#2804ac]">Create Organization</button>
        </form>
      </div>
    </div>
  );
